...
// ValToFloat64 将后端返回的数值型值转为float64。
...
// ValToInt64 将后端返回的数值型值转为int64。
// 不同方言后端对MIN/MAX(int列)的返回形态不一：int64/int/float64/[]byte/string等。
...
func ParseStrToInt64(s string) (int64, bool) {
	// 定宽文本/部分后端返回的数值串带首尾空格，与ParseStrToFloat64保持同样的裁剪口径
