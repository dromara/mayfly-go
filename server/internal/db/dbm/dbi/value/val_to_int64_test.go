...
// TestValToInt64BackendForms 各Go形态的转换结果矩阵（含各方言实测的窄整型）
func TestValToInt64BackendForms(t *testing.T) {
	tests := []struct {
		name string
		val  any
		want int64
...
// TestValToFloat64BackendForms 浮点归一矩阵（REAL/DOUBLE/FLOAT列与decimal文本）
func TestValToFloat64BackendForms(t *testing.T) {
	tests := []struct {
		name string
		val  any
		want float64
...
// TestValToBoolBackendForms 布尔归一矩阵：各方言bool列物理类型差异大（tinyint/bit/boolean/'t'文本）
func TestValToBoolBackendForms(t *testing.T) {
	tests := []struct {
		name string
		val  any
		want bool
