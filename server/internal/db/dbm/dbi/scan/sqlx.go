// Package scan 结果集→结构体反射映射（独立叶子组件，零 dbi 依赖）。
//
// 零 dbi 依赖的叶子工具：仅供 dbi/conn.go 的 StructQuery 调用，
// 实现复制自 sqlx 库（https://github.com/jmoiron/sqlx）并按本项目嵌套字段查找语义修正。
package scan
...
// All 将结果集全部行扫描至 dest 指向的切片（元素可为 map 或 struct）。
// structOnly 为 true 时 dest 元素非 struct 即报错。
func All(rows *sql.Rows, dest any, structOnly bool) error {
