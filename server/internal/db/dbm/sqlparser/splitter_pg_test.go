...
// pg 方言切割器测试（原 PgsqlSplitter 已下线，其独有能力由能力表驱动的统一切割器承接）：
// dollar-quote、E 串、嵌套块注释、双写引号；注释一律原样保留
func pgSplitAll(t *testing.T, sql string) []string {
	t.Helper()
	return splitAll(t, NewSplitter(tokenizer.PgConfig), sql)
}

func TestPgSplitterBasic(t *testing.T) {
	...
	// 纯注释不产生语句（注释内的分号不参与切割）
	assert.Empty(t, pgSplitAll(t, "-- comment; here\n/* block; */"))
}

func TestPgSplitterStrings(t *testing.T) {
...
// PG专属 E'...' 转义字符串：仅E串内反斜杠为转义符
func TestPgSplitterEscapeString(t *testing.T) {
	// \' 不结束字符串，分号在字符串内
	...
	// E'' 双写引号依然生效
	assert.Equal(t, []string{`insert into t values (E'it''s;x')`}, pgSplitAll(t, `insert into t values (E'it''s;x');`))
	// E'\'' ：被转义的引号不闭合，其后整段处于字符串内直至EOF未闭合 → 明确报错（pg词法行为一致）
	_, err := splitErrAll(t, NewSplitter(tokenizer.PgConfig), `insert into t values (E'\'); insert into t values ('x');`)
	assertUnterminated(t, err, 1, tokenizer.KindString)
	// 注释原样保留，其两侧空白不变
	assert.Equal(t, []string{`insert /* E */ into t values (E'a;b')`},
		pgSplitAll(t, `insert /* E */ into t values (E'a;b');`))
}

// PG专属 dollar-quoted 字符串：$$...$$ / $tag$...$tag$（函数体/DO块内分号不切割）
func TestPgSplitterDollarQuote(t *testing.T) {
	// 空tag
	assert.Equal(t, []string{`insert into t values ($$a;b$$)`}, pgSplitAll(t, `insert into t values ($$a;b$$);`))
	// 带tag
...
END $$;`
	...
	// 未闭合的 dollar-quote
	_, err := splitErrAll(t, NewSplitter(tokenizer.PgConfig), "select $$abc;")
	assertUnterminated(t, err, 1, tokenizer.KindDollarQuote)
}

// PG支持嵌套块注释，且注释文本原样保留
func TestPgSplitterNestedComment(t *testing.T) {
	assert.Equal(t, []string{"select /* /* ; */ ; */ 1"}, pgSplitAll(t, `select /* /* ; */ ; */ 1;`))
	assert.Equal(t, []string{"select /* outer /* inner ; */ ; */ 1", "select 2"},
		pgSplitAll(t, `select /* outer /* inner ; */ ; */ 1; select 2;`))
	// 嵌套注释跨行
	assert.Equal(t, []string{"select /* l1\n/* inner ; */\nl2 ; */ 1"}, pgSplitAll(t, "select /* l1\n/* inner ; */\nl2 ; */ 1;"))
	// 注释内的引号不进入字符串状态
	assert.Equal(t, []string{`select /* it's "quoted" ; */ 1`}, pgSplitAll(t, `select /* it's "quoted" ; */ 1;`))
}

func TestPgSplitterMultibyte(t *testing.T) {
...
// TestPgSplitterBadUtf8 非法UTF8字节容错：坏字节原样透传，不得中断解析导致后续语句丢失
func TestPgSplitterBadUtf8(t *testing.T) {
	...
	// 注释中的坏字节随注释原文保留
	assert.Equal(t, []string{"select 1", "-- \xff 注释\nselect 中文", "select 3"},
