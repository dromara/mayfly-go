...
SELECT t.name                                               AS TABLE_NAME,
       ...
       -- 计算列（computed column）的值由表达式派生，不可显式INSERT（SQL Server报“不能向计算列插入值”），
       -- 必须标记以便数据迁移/导出时从插入列集中剔除；is_computed在SQL Server 2005+均可用
       IS_GENERATED = CASE WHEN c.is_computed = 1 THEN 1 ELSE 0 END,
       -- 计算列定义原文取自sys.computed_columns（2005+可用）：不取则结构迁移只能把计算列建成普通列，
       -- 与INSERT阶段的剔除叠加会使该列静默变NULL（数据丢失），故必须连表达式一起取
       COMPUTED_DEF = ISNULL(cc.definition, ''),
       IS_PERSISTED = CASE WHEN cc.is_persisted = 1 THEN 1 ELSE 0 END,
       IS_PRIMARY_KEY = CASE
                            WHEN (SELECT COUNT(*)
