...
func mssqlTargetDialect() dbi.Dialect {
	...
	conn := &dbi.DbConn{Info: &dbi.DbInfo{Type: DbTypeMssql, Backend: meta}}
	return conn.GetDialect()
}

// 整数族映射：防止映射指错类型导致迁移数据截断
func TestMssqlConverter_IntMappings(t *testing.T) {
	engine := dbi.GetTypeEngine(DbTypeMssql)
	col := newCol()
	...
	assert.Equal(t, Tinyint, resolveTarget(t, engine, dbi.TCUnsignedInt1, col))
	assert.Equal(t, Smallint, resolveTarget(t, engine, dbi.TCUnsignedInt2, col))
...
func TestMssqlConverter_VarcharMaxLengthBoundary(t *testing.T) {
	...
	assert.Equal(t, Nvarchar, resolveTarget(t, engine, dbi.TCVarchar, col))
	assert.Equal(t, 4000, col.CharMaxLength)

	// 4001起只能落nvarchar(max)：不得截断为nvarchar(4000)（那会静默丢掉第4001个字符往后的内容）
	col2 := newCol()
	col2.CharMaxLength = 4001
	assert.Equal(t, NvarcharMax, resolveTarget(t, engine, dbi.TCVarchar, col2))
	...
	assert.Equal(t, NvarcharMax, resolveTarget(t, engine, dbi.TCVarchar, col3))
	col4 := newCol()
	col4.CharMaxLength = 65535
	assert.Equal(t, NvarcharMax, resolveTarget(t, engine, dbi.TCVarchar, col4))
	...
	col5 := newCol()
	assert.Equal(t, NvarcharMax, resolveTarget(t, engine, dbi.TCVarchar, col5))
	col6 := newCol()
	col6.CharMaxLength = -1 // SQL Server对max列回报-1，防御性同样归为不限长
	assert.Equal(t, NvarcharMax, resolveTarget(t, engine, dbi.TCVarchar, col6))
}
...
func TestMssqlConverter_BytesTypeBoundary(t *testing.T) {
	...
	col := newCol()
	assert.Equal(t, VarbinaryMax, resolveTarget(t, engine, dbi.TCLongblob, col))

	// 超过8000 → varbinary(max)
	col2 := newCol()
	col2.CharMaxLength = 16777215
	assert.Equal(t, VarbinaryMax, resolveTarget(t, engine, dbi.TCBlob, col2))
	assert.Equal(t, 0, col2.CharMaxLength)

	// 有长度且不超限 → varbinary(n)（保真，不用定长binary避免pad 0x00）
	col3 := newCol()
	col3.CharMaxLength = 16
	assert.Equal(t, Varbinary, resolveTarget(t, engine, dbi.TCBinary, col3))
	assert.Equal(t, Varbinary, resolveTarget(t, engine, dbi.TCVarbinary, col3))

	// blob族全部走归一化
	col4 := newCol()
	assert.Equal(t, VarbinaryMax, resolveTarget(t, engine, dbi.TCMediumblob, col4))
	col5 := newCol()
	assert.Equal(t, VarbinaryMax, resolveTarget(t, engine, dbi.TCVarbinary, col5))
}
...
func TestMssqlConverter_TextClearLength(t *testing.T) {
	...
	assert.Equal(t, NvarcharMax, resolveTarget(t, engine, dbi.TCText, col))
...
func TestMssqlConverter_RepresentativeMappings(t *testing.T) {
	...
	assert.Equal(t, Datetime2, resolveTarget(t, engine, dbi.TCDateTime, col))
	assert.Equal(t, Datetime2, resolveTarget(t, engine, dbi.TCTimestamp, col))
	// enum元数据回报的列长为各枚举值最大字节数（>0），按有界字符列处理
	enumCol := newCol()
	enumCol.CharMaxLength = 20
	assert.Equal(t, Nvarchar, resolveTarget(t, engine, dbi.TCEnum, enumCol))
	assert.Equal(t, NvarcharMax, resolveTarget(t, engine, dbi.TCJSON, col))
	...
	unbounded := newCol()
	assert.Equal(t, Decimal, resolveTarget(t, engine, dbi.TCDecimal, unbounded))
	...
	assert.Equal(t, Datetime2, resolveTarget(t, engine, dbi.TCDateTime, timeCol))
	assert.Equal(t, 3, timeCol.NumPrecision)
	assert.Equal(t, 0, timeCol.NumScale, "时间类型的标度无意义，必须清零避免拼出datetime2(3,2)")
	assert.Equal(t, Datetime2, resolveTarget(t, engine, dbi.TCTimestamp, &dbi.Column{NumPrecision: 9}))
	dateCol := newCol()
	dateCol.NumPrecision = 3
	resolveTarget(t, engine, dbi.TCDate, dateCol)
...
func TestMssqlConverter_RemainingMappings(t *testing.T) {
	...
	assert.Equal(t, Nchar, resolveTarget(t, engine, dbi.TCChar, charCol))
	assert.Equal(t, NvarcharMax, resolveTarget(t, engine, dbi.TCChar, newCol()))
	// TCNumeric（浮点/无约束数值）落float而非numeric：numeric不带精度等价numeric(18,0)会截断小数
	assert.Equal(t, Float, resolveTarget(t, engine, dbi.TCNumeric, newCol()))
	assert.Equal(t, Time, resolveTarget(t, engine, dbi.TCTime, newCol()))

	// mediumtext/longtext同样落nvarchar(max)并清空长度
	col := newCol()
	col.CharMaxLength = 500
	assert.Equal(t, NvarcharMax, resolveTarget(t, engine, dbi.TCMediumtext, col))
	assert.Equal(t, 0, col.CharMaxLength)

	col2 := newCol()
	col2.CharMaxLength = 500
	assert.Equal(t, NvarcharMax, resolveTarget(t, engine, dbi.TCLongtext, col2))
	assert.Equal(t, 0, col2.CharMaxLength)
}
...
func TestMssqlConvToTargetDbColumn_FromMysql(t *testing.T) {
	...
	// longblob（无长度，TCBinary）→ varbinary(max)，历史上曾生成 binary(1) 导致插入必失败
