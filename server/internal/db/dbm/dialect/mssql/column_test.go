...
//  2. 无限长类型必须归文本/大对象公共类型（TCLongtext/TCLongblob）迁出，与 oracle/dm 对
//     CLOB/TEXT 的处理同构；归 TCVarchar 会使异构目标按定长列建表。
...
import (
	...
	_ "mayfly-go/internal/db/dbm/dialect/mysql"    // 触发目标方言类型注册
	_ "mayfly-go/internal/db/dbm/dialect/postgres" // 触发目标方言类型注册
	_ "mayfly-go/internal/db/dbm/dialect/sqlite"   // 触发目标方言类型注册
)
...
func targetDialectOf(dt dbi.DbType) dbi.Dialect {
	conn := &dbi.DbConn{Info: &dbi.DbInfo{Type: dt, Backend: dbi.GetBackend(dt)}}
...
func TestUnboundedTypesMigrateAsText(t *testing.T) {
	...
	require.NotNil(t, dbi.GetBackend(DbTypeMssql))
	cases := []struct {
		src      string
		target   dbi.DbType
		wantType string
