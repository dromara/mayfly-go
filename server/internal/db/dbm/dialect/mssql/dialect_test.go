...
func TestMssqlGenTruncate(t *testing.T) {
	gen := newTestSQLGenerator()
	sqls := gen.GenTruncate("t_user")
	assert.Len(t, sqls, 1)
	assert.Contains(t, sqls[0], "TRUNCATE TABLE")
	assert.Contains(t, sqls[0], "t_user")
}

func TestMssqlGenBatchDelete_SinglePK(t *testing.T) {
	gen := newTestSQLGenerator()
