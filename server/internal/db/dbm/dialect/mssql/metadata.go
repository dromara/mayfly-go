...
var (
	_ dbi.ServerInfo       = (*MssqlMetadata)(nil)
	_ dbi.MetadataProvider = (*MssqlMetadata)(nil)
)
...
func (md *MssqlMetadata) GetDbServer() (*dbi.DbServer, error) {
	_, res, err := md.di.Query("SELECT @@VERSION as version")
	if err != nil {
		return nil, err
	}
	if len(res) == 0 {
...
func (md *MssqlMetadata) GetTables(tableNames ...string) ([]dbi.Table, error) {
	dialect := md.di.GetDialect()
	schema := md.di.CurrentSchema()
	...
	_, res, err = md.di.Query(sql, schema)
	if err != nil {
		return nil, err
	}
...
func (md *MssqlMetadata) GetColumns(tableNames ...string) ([]dbi.Column, error) {
	...
	for _, re := range res {
		...
		md.di.GetDbDataType(column.DataType).FixColumn(&column)
...
func (md *MssqlMetadata) GetTableDDL(tableName string, dropBeforeCreate bool) (string, error) {
	return dbi.GenTableDDL(md.di.GetDialect(), md, tableName, dropBeforeCreate)
}
