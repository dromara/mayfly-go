...
var (
	_ dbi.ServerInfo       = (*DMMetadata)(nil)
	_ dbi.MetadataProvider = (*DMMetadata)(nil)
)
...
func (dd *DMMetadata) GetDbServer() (*dbi.DbServer, error) {
	_, res, err := dd.di.Query("select * from v$instance")
	if err != nil {
		return nil, err
	}
	if len(res) == 0 {
...
func (dd *DMMetadata) GetDbNames() ([]string, error) {
	_, res, err := dd.di.Query("SELECT name AS DBNAME FROM v$database")
	if err != nil {
		return nil, err
	}
...
func (dd *DMMetadata) GetColumns(tableNames ...string) ([]dbi.Column, error) {
	...
	for _, re := range res {
		...
		dd.di.GetDbDataType(column.DataType).FixColumn(&column)
...
func (dd *DMMetadata) GetTableDDL(tableName string, dropBeforeCreate bool) (string, error) {
	return dbi.GenTableDDL(dd.di.GetDialect(), dd, tableName, dropBeforeCreate)
}
