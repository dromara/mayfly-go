...
func (cd *ClickHouseDialect) GetSQLGenerator() dbi.SQLGenerator {
	return &ClickHouseSQLGenerator{BaseSQLGenerator: dbi.BaseSQLGenerator{QuoterFn: cd.Quoter}, dialect: cd}
}
...
// GenTruncate ClickHouse 不支持 TRUNCATE TABLE，返回空切片并记录日志告警
func (csg *ClickHouseSQLGenerator) GenTruncate(tableName string) []string {
	logx.Warnf("ClickHouse does not support TRUNCATE TABLE, skipping truncate for table [%s]", tableName)
	return nil
}

// GenBatchDelete 生成按主键/唯一键批量删除语句（删除 NOT IN 给定值的记录）。
// 用于硬删除/软删除同步：源库不存在但目标库存在的记录需删除。
// ClickHouse 使用 ALTER TABLE t DELETE WHERE（DELETE 后不重复表名），
// 无法直接复用 dbi.BuildBatchDelete，改用 BuildBatchDeleteWhere 构建 WHERE 子句。
func (csg *ClickHouseSQLGenerator) GenBatchDelete(tableName string, keyColumns []string, keyValues [][]any, targetTableMeta *dbi.TargetTableMeta) []string {
	...
	where := dbi.BuildBatchDeleteWhere(keyColumns, tuples, quote)
	return []string{fmt.Sprintf("ALTER TABLE %s DELETE WHERE %s", quote(tableName), where)}
}
