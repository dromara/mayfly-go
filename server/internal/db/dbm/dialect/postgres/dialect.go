...
func (pd *PgsqlDialect) CopyTable(copy *dbi.DbCopyTable) error {
	...
	// 执行根据旧表创建新表
	if _, err := pd.di.Exec(fmt.Sprintf("create table %s (like %s)", quote(newTableName), quote(tableName))); err != nil {
		return err
	}
	...
	if copy.CopyData {
		gox.Go(func() {
			if _, err := pd.di.Exec(fmt.Sprintf("insert into %s select * from %s", quote(newTableName), quote(tableName))); err != nil {
	...
	_, res, err := pd.di.Query("select column_name from information_schema.columns where table_name = $1 and column_default like 'nextval%'", tableName)
	if err != nil {
		return err
	}
	...
	for _, re := range res {
		...
		if colName != "" {
			// 查询自增列当前最大值
			_, maxRes, err := pd.di.Query(fmt.Sprintf("select max(%s) max_val from %s", quote(colName), quote(tableName)))
			if err != nil {
				return err
			}
			if len(maxRes) == 0 {
			...
			// 创建自增序列，当前最大值为旧表最大值
			if _, err := pd.di.Exec(fmt.Sprintf("CREATE SEQUENCE %s START %d INCREMENT 1", quote(newSeqName), maxVal)); err != nil {
				return err
			}
			// 将新表的自增主键序列与主键列相关联：nextval参数是字符串字面量，其内容会被pg当作对象名再解析一次，
			// 故必须先按标识符引用（否则含特殊字符/大写的序列名会报 invalid name syntax 或被downcase后找不到），
			// 再按字符串字面量语义转义单引号
			if _, err := pd.di.Exec(fmt.Sprintf("alter table %s alter column %s set default nextval('%s')", quote(newTableName), quote(colName), dbi.QuoteEscape(quote(newSeqName)))); err != nil {
				return err
			}
		}
	}
