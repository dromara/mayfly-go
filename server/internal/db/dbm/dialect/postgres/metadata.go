...
func (pd *PgsqlMetadata) GetDbServer() (*dbi.DbServer, error) {
	_, res, err := pd.di.Query("SELECT version() as server_version")
	if err != nil {
		return nil, err
	}
	if len(res) == 0 {
	...
	dbi.ParseDbVersion(ds)
	return ds, nil
}

// GetCompatibleDbVersion 兼容库方言返回对应的 PostgreSQL 兼容版本语义
func (pd *PgsqlMetadata) GetCompatibleDbVersion() dbi.DbVersion {
	switch pd.di.Type {
	case DbTypeGauss:
		return "gauss"
...
func (pd *PgsqlMetadata) GetDbNames() ([]string, error) {
	_, res, err := pd.di.Query("SELECT datname AS dbname FROM pg_database WHERE datistemplate = false AND has_database_privilege(datname, 'CONNECT')")
	if err != nil {
		return nil, err
	}
...
func (pd *PgsqlMetadata) GetColumns(tableNames ...string) ([]dbi.Column, error) {
	...
	for _, re := range res {
		...
		pd.di.GetDbDataType(column.DataType).FixColumn(&column)
...
func (pd *PgsqlMetadata) markGeneratedColumns(tableNamesLiteral string, columns []dbi.Column) {
	...
	srcDbType := cast.ToString(pd.di.Type)
	for _, re := range res {
...
func (pd *PgsqlMetadata) GetTableDDL(tableName string, dropBeforeCreate bool) (string, error) {
	return dbi.GenTableDDL(pd.di.GetDialect(), pd, tableName, dropBeforeCreate)
}
