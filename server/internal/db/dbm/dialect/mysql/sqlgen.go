...
func (msg *SQLGenerator) GenInsert(tableName string, columns []dbi.Column, values [][]any, duplicateStrategy int, targetTableMeta *dbi.TargetTableMeta) []string {
	...
	return collx.AsArray[string](fmt.Sprintf("insert into %s %s VALUES \n%s ON DUPLICATE KEY UPDATE %s",
		quote(tableName), columnStr, strings.Join(valuesStrs, ",\n"), updateSet))
}

// genOnDuplicateKeyUpdateSet 生成 MySQL ON DUPLICATE KEY UPDATE 的 SET 子句。
// 排除唯一键列自身（无意义更新）和生成列（不可显式写入）
func (msg *SQLGenerator) genOnDuplicateKeyUpdateSet(columns []dbi.Column, targetTableMeta *dbi.TargetTableMeta) string {
	...
	for _, col := range columns {
		...
		if dbi.PreservableGeneratedColumn(col, DbTypeMysql) || col.IsGenerated {
			continue
		}
		quotedName := quote(col.ColumnName)
		sets = append(sets, fmt.Sprintf("%s = VALUES(%s)", quotedName, quotedName))
	}
