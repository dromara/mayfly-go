...
func TestMysqlGenInsert(t *testing.T) {
	...
	// 更新冲突：UPSERT（INSERT ... ON DUPLICATE KEY UPDATE），无元信息时退化为 INSERT IGNORE
	sqls = gen.GenInsert("t1", columns, values, dbi.DuplicateStrategyUpdate, nil)
	assert.Len(t, sqls, 1)
	assert.Contains(t, sqls[0], "insert ignore into")

	// 有TargetTableMeta时生成 ON DUPLICATE KEY UPDATE 子句
...
func TestMysqlGenTruncate(t *testing.T) {
	gen := newTestSQLGenerator()
...
func TestMysqlGenBatchDelete_SinglePK(t *testing.T) {
	gen := newTestSQLGenerator()
...
func TestMysqlGenBatchDelete_CompositePK(t *testing.T) {
	gen := newTestSQLGenerator()
...
func TestMysqlGenBatchDelete_EmptyInput(t *testing.T) {
	...
	assert.Nil(t, gen.GenBatchDelete("t", []string{}, [][]any{{1}}, nil))
	assert.Nil(t, gen.GenBatchDelete("t", []string{"id"}, nil, nil))
