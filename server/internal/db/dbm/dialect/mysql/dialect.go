...
func (md *MysqlDialect) CopyTable(copy *dbi.DbCopyTable) error {
	...
	// 复制表结构创建表
	if _, err := md.di.Exec(fmt.Sprintf("create table %s like %s", quote(newTableName), quote(tableName))); err != nil {
		return err
	}
	...
	if copy.CopyData {
		gox.Go(func() {
			if _, err := md.di.Exec(fmt.Sprintf("insert into %s select * from %s", quote(newTableName), quote(tableName))); err != nil {
