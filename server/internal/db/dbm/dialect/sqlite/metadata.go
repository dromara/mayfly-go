...
func (sd *SqliteMetadata) GetColumns(tableNames ...string) ([]dbi.Column, error) {
	...
	for i := 0; i < len(tableNames); i++ {
		...
		for _, re := range res {
			...
			sd.di.GetDbDataType(column.DataType).FixColumn(&column)
...
func (sd *SqliteMetadata) GetTableDDL(tableName string, dropBeforeCreate bool) (string, error) {
	...
	if dropBeforeCreate {
		builder.WriteString(fmt.Sprintf("DROP TABLE IF EXISTS %s; \n\n", sd.di.GetDialect().Quoter().QuoteIdent(tableName)))
	}

	_, res, err := sd.di.Query("select sql from sqlite_master WHERE tbl_name=? order by type desc", tableName)
	if err != nil {
		return "", err
	}
