...
func (sd *SqliteDialect) CopyTable(copy *dbi.DbCopyTable) error {
	...
	ddl, err := sd.di.Metadata().GetTableDDL(tableName, false)
	if err != nil {
		return err
	}
	// 生成建表语句
	...
	if copy.CopyData {
		gox.Go(func() {
			// 执行插入语句
			if _, err := sd.di.Exec(fmt.Sprintf("INSERT INTO \"%s\" SELECT * FROM \"%s\"", newTableName, tableName)); err != nil {
