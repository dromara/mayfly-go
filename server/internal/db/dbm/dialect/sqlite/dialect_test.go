...
func TestSqliteGenInsert_IgnoreAndReplace(t *testing.T) {
	...
	// Update策略：无TargetTableMeta时退化为 INSERT OR IGNORE
...
func TestSqliteAffinityTypeRegistered(t *testing.T) {
	...
	tests := []struct {
		...
		{"integer", dbi.TCInt8},
		{"tinyint", dbi.TCInt8},
		{"varchar", dbi.TCVarchar},
		{"nvarchar", dbi.TCVarchar},
		{"char", dbi.TCChar},
		{"text", dbi.TCText},
		{"clob", dbi.TCText},
		{"numeric", dbi.TCDecimal},
		{"decimal", dbi.TCDecimal},
		{"float", dbi.TCNumeric},
		{"double", dbi.TCNumeric},
		{"real", dbi.TCNumeric},
		{"timestamp", dbi.TCTimestamp},
		{"datetime", dbi.TCDateTime},
		{"date", dbi.TCDate},
		{"blob", dbi.TCBlob},
	}
