...
func newTestSQLGenerator() *SQLGenerator {
	// 触发oracle列类型注册（TypeEngine注册包含向dbDataTypes的同步）
	dbi.GetTypeEngine(DbTypeOracle)
	return &SQLGenerator{BaseSQLGenerator: dbi.BaseSQLGenerator{QuoterFn: (&OracleDialect{}).Quoter}, Dialect: &OracleDialect{}}
}
...
func TestOracleGenTruncate(t *testing.T) {
	gen := newTestSQLGenerator()
	sqls := gen.GenTruncate("T_USER")
	assert.Len(t, sqls, 1)
	assert.Equal(t, "TRUNCATE TABLE \"T_USER\"", sqls[0])
}

func TestOracleGenBatchDelete_SinglePK(t *testing.T) {
	gen := newTestSQLGenerator()
	sqls := gen.GenBatchDelete("T_USER", []string{"ID"}, [][]any{{1}, {2}}, nil)
	assert.Len(t, sqls, 1)
	assert.Contains(t, sqls[0], "DELETE FROM \"T_USER\" WHERE")
	assert.Contains(t, sqls[0], "\"ID\" NOT IN (")
	assert.Contains(t, sqls[0], "SELECT '1' \"ID\" FROM dual")
	assert.Contains(t, sqls[0], "UNION ALL")
}

func TestOracleGenBatchDelete_EmptyInput(t *testing.T) {
	gen := newTestSQLGenerator()
