...
select a.TABLE_NAME,
       ...
       COALESCE(d.BYTES, 0) as DATA_LENGTH,
       0          as INDEX_LENGTH,
       a.NUM_ROWS as TABLE_ROWS
from SYS.ALL_TABLES a
         left join SYS.ALL_TAB_COMMENTS b on b.TABLE_NAME = a.TABLE_NAME AND b.OWNER = a.OWNER
         left join SYS.ALL_OBJECTS c on c.OBJECT_TYPE = 'TABLE' AND c.OWNER = a.OWNER AND c.OBJECT_NAME = a.TABLE_NAME
         left join SYS.USER_SEGMENTS d on d.SEGMENT_TYPE = 'TABLE' AND d.SEGMENT_NAME = a.TABLE_NAME
where a.owner = (SELECT sys_context('USERENV', 'CURRENT_SCHEMA') FROM SYS.dual)
{{if .tableNames}}
...
SELECT ai.INDEX_NAME                          AS INDEX_NAME,
       ...
       (SELECT comments
        FROM SYS.ALL_IND_COLUMNS aic
            LEFT JOIN SYS.ALL_COL_COMMENTS acc ON aic.column_name = acc.column_name
...
WHERE ai.OWNER = (SELECT sys_context('USERENV', 'CURRENT_SCHEMA') FROM SYS.dual)
  AND ai.table_name = '%s'
...
FROM SYS.ALL_TAB_COLUMNS a
         LEFT JOIN SYS.ALL_COL_COMMENTS b
         ...
         LEFT JOIN (select ac.TABLE_NAME, ac.OWNER, cc.COLUMN_NAME, 1 as pri
                    from SYS.ALL_CONSTRAINTS ac
                             join SYS.ALL_CONS_COLUMNS cc on cc.CONSTRAINT_NAME = ac.CONSTRAINT_NAME AND cc.OWNER = ac.OWNER
