...
// TestITFallbackSelectWithLeadingComment 兜底字符串匹配对前置注释SELECT的分类：
// ExecuteSql在解析失败时用getSqlPrefix（首10字符）兜底，依赖切割器已剥离注释——
// 本用例验证切割器输出确实以select开头（保证兜底分类不会把查询误判为非查询）
func TestITFallbackSelectWithLeadingComment(t *testing.T) {
	...
	assert.Contains(t, stmts[0], "select", "切割器应剥离前置注释，保证兜底前缀匹配以select开头")
}
