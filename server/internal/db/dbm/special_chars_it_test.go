...
func TestITMysqlEscapeSpecialRoundtrip(t *testing.T) {
	...
	importScript(t, conn, strings.NewReader(strings.ReplaceAll(script, table, importTable)))
	...
	importScript(t, sqConn, strings.NewReader(script2))
	...
	mustExec(t, conn, "DROP TABLE IF EXISTS `"+importTable+"`")
	mustExec(t, conn, "CREATE TABLE `"+importTable+"` LIKE `"+table+"`")
	importScript(t, conn, strings.NewReader(strings.ReplaceAll(nulScript, table, importTable)))
