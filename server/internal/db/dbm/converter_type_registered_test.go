...
//
// 每个方言的 TypeEngine 对任意列形态下产出的目标类型，必须能在该方言自己的
...
//
// 本用例遍历 TypeEngine 的所有 TypeCategory 规则，新增方言、新增 TC 或新增目标类型时自动纳入校验。
...
// converterColumnShapes 转换方法入参的代表性列形态：长度/精度组合覆盖各规则的分支边界
...
func TestConverterOutputTypesRegistered(t *testing.T) {
	for _, dt := range expectedDbTypes {
		...
		if engine == nil {
			// 别名方言（如mariadb/gauss）共享主方言的TypeEngine，不单独注册
			continue
		}

		for _, cat := range dbi.AllTypeCategories() {
			cat := cat
			t.Run(string(dt)+"/TC"+string(rune('0'+cat)), func(t *testing.T) {
				for _, shape := range converterColumnShapes {
					shape := shape
					col := shape
					target, err := engine.ResolveTarget(cat, &col)
					if err != nil {
						// 该类别未注册规则且无降级，跳过
						return
					}
					require.NotNil(t, target, "[%s] TypeCategory %d 对列 %+v 返回nil（异构迁移会静默丢列类型）",
						dt, cat, shape)
					require.NotEmpty(t, target.Name, "[%s] TypeCategory %d 返回无名类型", dt, cat)
					...
					assert.NotSame(t, dbi.DefaultDbDataType, resolved,
						"[%s] TypeCategory %d 产出的类型[%s]未在类型注册表中，INSERT值转SQL将静默回退默认字符串类型",
						dt, cat, target.Name)
