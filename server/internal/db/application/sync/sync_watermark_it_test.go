...
func countSyncRows(t *testing.T, conn *dbi.DbConn, table string) int {
	...
	cnt, _ := value.ValToInt64(rows[0]["cnt"])
	return int(cnt)
}
...
func TestITDataSyncWatermark(t *testing.T) {
	...
	require.NoError(t, app.doDataSync(ctx, fullSql, task, log1, &SyncMetrics{}))
	...
	require.NoError(t, app.doDataSync(ctx, incrSql, task2, log2, &SyncMetrics{}))
	...
	err := app.doDataSync(ctx, incrSql2, task3, log3, &SyncMetrics{})
	...
	finalWatermark, _ := value.ValToInt64(fake.updated[len(fake.updated)-1].UpdFieldVal)
