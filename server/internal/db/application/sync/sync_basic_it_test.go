...
func TestITDataSyncSrc2TargetDb(t *testing.T) {
	...
	app := &DataSyncAppImpl{}
	// 首次全量同步
	require.NoError(t, app.srcData2TargetDb(context.Background(), srcRes, buildTestSyncExecContext(fieldMap, task, dstConn, targetColumns, targetMeta)))
	...
	require.NoError(t, err)
	require.NoError(t, app.srcData2TargetDb(context.Background(), srcRes2, buildTestSyncExecContext(fieldMap, task, dstConn, targetColumns, targetMeta)))
...
// buildTestSyncExecContext 构建集成测试用的 syncExecContext（与生产 SyncBatch 等价）
func buildTestSyncExecContext(fieldMap []map[string]string, task *entity.DataSyncTask, targetConn *dbi.DbConn, targetColumns []dbi.Column, targetMeta *dbi.TargetTableMeta) *syncExecContext {
	return &syncExecContext{
		fieldMap:          fieldMap,
		task:              task,
		targetConn:        targetConn,
		targetColumns:     targetColumns,
		targetTableMeta:   targetMeta,
		metrics:           &SyncMetrics{},
		failedRowRecorder: NewFailedRowRecorder(1000),
	}
}
...
func syncToInt64(v any) int64 {
	...
	case float64:
		return int64(val)
	default:
		// 后端可能返回string/[]byte形式的数字（如"100"），统一解析
