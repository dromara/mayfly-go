...
func TestITDataSyncHeteroFull(t *testing.T) {
	for _, spec := range syncHeteroSpecs {
		...
		t.Run(spec.name, func(t *testing.T) {
			...
			app := &DataSyncAppImpl{}
			require.NoError(t, app.srcData2TargetDb(context.Background(), srcRes, buildTestSyncExecContext(fieldMap, task, tgtConn, targetColumns, targetMeta)),
				"[%s] 全量同步失败", spec.name)
			...
			_, srcRes2, err := srcConn.Query(fmt.Sprintf("SELECT * FROM %s ORDER BY id",
				quoteSync(t, srcConn, "it_sync_hetero")))
			require.NoError(t, err)
			require.NoError(t, app.srcData2TargetDb(context.Background(), srcRes2, buildTestSyncExecContext(fieldMap, task, tgtConn, targetColumns, targetMeta)))
