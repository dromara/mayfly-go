...
func TestITDataSyncSrc2TargetDb(t *testing.T) {
	...
	targetMeta := buildTargetTableMeta(dstConn, "it_sync_target", targetColumns)
	...
	app := &dataSyncAppImpl{}
	// 首次全量同步
	require.NoError(t, app.srcData2TargetDb(srcRes, fieldMap, "", task, dstConn, targetColumns, targetMeta))
	...
	require.NoError(t, err)
	require.NoError(t, app.srcData2TargetDb(srcRes2, fieldMap, "", task, dstConn, targetColumns, targetMeta))
...
func TestITDataSyncToMysqlTarget(t *testing.T) {
	...
	targetMeta := buildTargetTableMeta(dstConn, "it_sync_m_target", targetColumns)
...
func TestITDataSyncToSqliteTarget(t *testing.T) {
	...
	targetMeta := buildTargetTableMeta(dstConn, "it_sync_sq_target", targetColumns)
