...
func setupVerifyPair(t *testing.T, n int) (srcConn, tgtConn *dbi.DbConn) {
	...
	require.NoError(t, (&transfer.DbTransferAppImpl{}).ImportDumpStream(context.Background(), 0, tgtConn,
...
func TestITVerifyTable_AllMatch(t *testing.T) {
	...
	res := (&transfer.DbTransferAppImpl{}).VerifyTable(context.Background(), srcConn, tgtConn, verifyItTable)
...
func TestITVerifyTable_NoPkTable(t *testing.T) {
	...
	res := (&transfer.DbTransferAppImpl{}).VerifyTable(context.Background(), conn, conn, table)
	require.Empty(t, res.Err)
	assert.True(t, res.CountMatch)
