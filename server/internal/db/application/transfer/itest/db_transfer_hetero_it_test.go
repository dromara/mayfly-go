...
// 异构数据迁移 端到端真实链路集成测试（全类型+大数据量，四方向）：
//
//	mysql→pg、pg→mysql、mysql→sqlite、mysql→mssql
//
// 真实链路：源库建表写数据（边界行+bulk行） → transfer.DumpDbScript(TargetDbType=目标方言)导出到文件
...
var hetSpecs = []hetSpec{
	...
	{
		// mssql作为异构目标：字符列必须落nvarchar且字面量带N前缀，否则utf8mb4源库的
		// 中文/emoji会在SQL Server端按库代码页静默转为'?'（LEN不变、无任何报错）；
		// 二进制列必须命中varbinary(max)的类型注册，否则值会被输出为'00ff'字面量而按ASCII逐字节损坏
		name:       "mysql->mssql",
		table:      "it_het_c",
		srcDialect: "mysql",
		srcDDL: "CREATE TABLE `it_hetero` (id BIGINT PRIMARY KEY, c_int INT, c_big BIGINT, c_f32 FLOAT, c_f64 DOUBLE, " +
...
// TestITHeteroMigrate 异构迁移四方向（全类型+大数据量）
func TestITHeteroMigrate(t *testing.T) {
	...
	for _, spec := range hetSpecs {
		...
		t.Run(spec.name, func(t *testing.T) {
			...
			require.NoError(t, transfer.DumpDbScript(context.Background(), srcConn, &dto.DumpDb{
