...
import type { DbMaskRule } from '@/views/ops/db/types';
...
const switchWarns = (spy: ReturnType<typeof vi.spyOn>) =>
    spy.mock.calls.map((c: unknown[]) => c.join(' ')).filter((s: string) => s.includes('active-value or inactive-value'));
...
const i18nMissWarns = (spy: ReturnType<typeof vi.spyOn>) =>
    spy.mock.calls.map((c: unknown[]) => c.join(' ')).filter((s: string) => s.includes('locale messages'));

/** 挂载编辑抽屉（openAtMount=false 时模拟真实点击：先关闭挂载再置 visible） */
const openEdit = async (data: DbMaskRule | null, openAtMount: boolean) => {
