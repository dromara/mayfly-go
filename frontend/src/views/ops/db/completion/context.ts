...
import { createCompletionQuoter, findWrappingQuote } from './quoter';
...
export async function buildCompletionContext(model: editor.ITextModel, position: Position, dbId: number, db: string, dbs: string[] = [], dbType: string): Promise<SqlCompletionContext> {
    ...
    const capabilities = getDialectCapabilities(dialect);
    ...
    const stmtAt = extractStatementAt(model.getValue(), model.getOffsetAt(position), capabilities.sqlSplitOptions);
    ...
    // 方言引用符与光标单词包裹状态
    const quotePairs = capabilities.quotePairs;
