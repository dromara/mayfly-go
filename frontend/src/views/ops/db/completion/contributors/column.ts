...
import { columnSuggestions, tableSuggestions } from '../suggestions';
 ...
 *
 * 建议项（suggestions.ts 产出）的 insertText 为裸名，统一经 quoteSuggestions 按方言包裹，
...
async function contributeDotTrigger(ctx: SqlCompletionContext) {
    ...
    if (dotIdx > 0) {
        ...
        if (dbName) {
            ...
            const items = await columnSuggestions(ctx.dbInst, dbName, tableName, ctx.range);
            return { suggestions: quoteSuggestions(ctx.quoteIdentifier, items), exclusive: true };
        }
    }
    ...
    if (dbName) {
        const items = await tableSuggestions(ctx.dbInst, dbName, ctx.range);
        return { suggestions: quoteSuggestions(ctx.quoteIdentifier, items), exclusive: true };
    }
