...
/** DbForm 已上移到 types/entity.ts（它同时是 confirm 事件父级处理函数的入参类型） */
...
const emit = defineEmits<{
    /** 取消编辑，父级关闭弹窗 */
    cancel: [];
    /** 保存成功，回传表单（父级补 instanceId 后落库并刷新列表） */
    confirm: [form: DbForm];
}>();
...
const getAllDatabase = async (authCertName: string) => {
    try {
        ...
        // 以 SID/Service Name 连接的库（oracle 系）查不出库名列表时，回落到实例 SID
        let instance = state.instances.find((item) => item.id === state.form.instanceId) as (DbInstance & { sid?: string }) | undefined;
        if (instance && getDialectCapabilities(getDbDialect(instance.type)).connectDescriptor === 'sid_service' && dbs.length === 0) {
