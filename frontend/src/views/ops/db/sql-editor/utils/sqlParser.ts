...
import { defaultSplitOptions } from '../../dialect/shared/capabilities';
import type { SqlSplitOptions } from '../../dialect/types';
...
// SQL 切割语义的类型定义与各方言预设均由 dialect 层持有（方言词法特征的唯一事实源）；
// 此处 re-export 以保持既有导入路径可用，切割状态机本身不感知任何具体方言
export type { SqlBlockMode, SqlSplitOptions } from '../../dialect/types';
export { defaultSplitOptions };

/**
 * 获取指定方言的切割选项。
 *
 * 直接读取方言能力声明，不再维护 dbType → 选项 的平行映射表：新增方言只需在自身文件声明
 * sqlSplitOptions，切割器与本查询入口均零修改（开闭原则）。
 *
 * 未注册的 dbType 回退到 MySQL 方言，与 getDbDialect 的回退行为保持一致。
 * （历史实现此处返回 {} 即标准 SQL 语义，而 DDL 生成却回退 MySQL 方言，两者不一致，现已修正）
 *
 * @param dbType 数据库类型，见 DbType
 */
export function getSqlSplitOptions(dbType?: string): SqlSplitOptions {
    return getDialectCapabilities(getDbDialect(dbType ?? '')).sqlSplitOptions;
}
 ...
 */
function readDollarTag(sql: string, i: number): string {
...
type ScanZone = 'code' | 'string' | 'identifier' | 'comment';
