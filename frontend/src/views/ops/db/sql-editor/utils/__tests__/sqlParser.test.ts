...
describe('splitSqlStatements 方言选项（数据安全回归）', () => {
    ...
    it('未注册方言回退 MySQL 切割语义（与 getDbDialect 回退契约一致）', () => {
        // 方言解析只有一条路径（版本方言 → 基础方言 → 回退方言）：未注册类型不按「空选项」处理，
        // 否则切割语义会与其他 getDbDialect 消费方分叉，重新长出平行映射表。
        // 中性缺省语义（defaultSplitOptions）由上方「默认选项…」系列用例直接传空选项覆盖。
        expect(getSqlSplitOptions('unknownDb')).toEqual(getSqlSplitOptions(DbType.mysql));
        expect(getSqlSplitOptions()).toEqual(getSqlSplitOptions(DbType.mysql));
    });
