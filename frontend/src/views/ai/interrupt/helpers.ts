/**
 * 中断恢复公共辅助函数
 * 对齐 tokhub interrupts/interruptHelpers.ts
 *
 * 提供与中断类型无关的通用操作：
 * - stateFromInterruptEvent：从 tool_call item extra.interrupt 构造历史恢复状态
 *   （对齐 tokhub buildBaseInterruptInfo 公共构造器）
 * - createInterruptHandler：标准 handler 工厂（对齐 tokhub approval 家族工厂
...
/**
 * 从 extra.interrupt（后端持久化于 tool_call item extra 列的 InterruptInfo，对齐 tokhub）
 * 构造公共历史恢复状态
 */
export function stateFromInterruptEvent(
    ...
    return {
        ...
        // 恢复决策内嵌 interrupt 对象（对齐 tokhub）：归一化具体决策类型，
...
/**
 * 创建标准中断处理器（对齐 tokhub approval 家族工厂）
 *
 * 统一提供：
...
export function createInterruptHandler(
    ...
    return {
        ...
        buildFromHistoryItem(item: TurnItem, itemExtra?: Record<string, unknown>): InterruptState | null {
            ...
            // 中断信息存于 tool_call item 的 extra 列（对齐 tokhub {"interrupt": InterruptInfo}）
...
/**
 * 决策解释委托 + 兜底（对齐 tokhub interpretDecisionWithFallback）
 *
