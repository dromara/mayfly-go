...
// 定义子组件向父组件传值/事件
const emit = defineEmits(['get', 'clear']);

// 引入组件
const IconList = defineAsyncComponent(() => import('@/components/icon-selector/list.vue'));
...
const initFontIconData = async (name: string) => {
    if (name === 'ele') {
        ...
        await initIconfont.ele().then((res: string[]) => {
            state.fontIconList.ele = res;
        });
    } else if (name === 'other') {
        // 本地导入svg图标, icon xxx
        state.fontIconList.other = initIconfont.other() as string[];
    }
...
const onColClick = (v: string) => {
    ...
    modelValue.value = state.fontIconPrefix;
    inputWidthRef.value?.focus();
};
// 清空当前点击的 icon 图标
